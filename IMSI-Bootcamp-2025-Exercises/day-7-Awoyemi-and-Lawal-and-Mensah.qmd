---
title: "Day 7: R and Quarto Exercises"
subtitle: "IMSI Summer Data Science Workshop Series 2025"
author: "Boluwatife Awoyemi, Maruf Lawal, Sarah Mensah"
format: html
editor: visual
execute: 
  echo: true
...
# Exercise 1: Got Quadratic?

## (a) **Defining a Custom Function**
...
## (b) **Variable declaration and re-use in function parameters**
...
y_quad = quadratic(x, a, b, c)

y_quad

```

## (c) **Generating a graphic**

```{r}

library(ggplot2)

my_df = data.frame(x = x, y = y_quad)
...
ggplot(data = my_df) +
  ...
  labs(
    title = "Visualizing a Quadratic Function",
    subtitle = paste0("with parameters a = ", a, ", b = ", b, ", c = ", c),
    y = "Output from f(x)",
    x = "Values of x input"
  ) +
  theme_bw()
...
# Exercise 2: The Recycler

## (a) Exploring a recycling operation
...
## (b) Describing recycling

```{r}

z = y * rep(x, length.out = 2)

z

```

R applies the rep() function to the specific variable by repeating the vector variables length.out times to match the length of the multiplying vector.
...
Since x is not a multiple of the new y vector, a warning is given and the result z = x \* y only returns a vector the same length as y, with x not evenly repeated.

## (d) Alternating Current

```{r}

alt_vec = rep(c(-1, 1), length.out = 27)

alt_vec

```

## (e) Special Sequencing

```{r}

k = rep(c(-1, 1, -1), length.out = 16) * c(1:16)
...
my_runif = function(n, start_val, end_val, lcg_a, lcg_c, lcg_m, lcg_seed) {
  ...
  for (i in 1:n) {
    ...
    U[i] = start_val + (X[i] / (lcg_m - 1)) * (end_val - start_val)
...
# my_rtri(n, a = 10, b = 20, c = 15, lcg_a, lcg_c, lcg_m, lcg_seed)
...
grade_dist = uiucdata::grade_dist
# head(grade_dist)

grade_dist_reduced = grade_dist[, -c(5:7)]
...
no_missing_obs = nrow(grade_dist_reduced) - nrow(grade_dist_complete)
no_missing_obs
...
The output looks like a plot of a normal distribution of a given population/sample.So the functions calculate mean and standard deviation of a given data, standardize a given value based on the data distribution and plots this against generated normal distribution with mean = 0 and standard deviation = 1.
